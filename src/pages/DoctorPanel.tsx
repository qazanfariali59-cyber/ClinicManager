...
export default function DoctorPanel({ user, onLogout }: DoctorPanelProps) {
  ...
  const tabs = [
    { id: 'quick-referral', label: 'ارجاع', icon: ArrowRight },
    { id: 'appointment', label: 'نوبت', icon: CalendarDays },
    { id: 'my-referrals', label: 'ارجاعاتم', icon: List },
    ...(isDoctor ? [{ id: 'my-patients', label: 'بیمارانم', icon: Users }] : []),
  ];
  ...
  return (
    <div className="app-shell" dir="rtl">
      <AppTopBar user={user} title={isDoctor ? 'پنل پزشک' : 'پنل همکار'} onLogout={onLogout} />

      {/* Stats strip */}
      <div className="bg-[#0d1b2a] px-4 pb-4">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-white/6 border border-cyan-400/15 rounded-2xl p-3 flex items-center gap-2">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0"
              style={{ background: 'rgba(0,232,214,0.15)' }}>
              <TrendingUp size={14} className="text-cyan-400" />
            </div>
            <div>
              <p className="text-[9px] text-white/40">دریافت‌شده</p>
              <p className="text-sm font-bold text-cyan-400">{totalPaid.toLocaleString('fa-IR')}</p>
            </div>
          </div>
          <div className="bg-white/6 border border-amber-400/15 rounded-2xl p-3 flex items-center gap-2">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0"
              style={{ background: 'rgba(251,191,36,0.15)' }}>
              <Clock size={14} className="text-amber-400" />
            </div>
            <div>
              <p className="text-[9px] text-white/40">در انتظار</p>
