...
export default function AppointmentForm({ currentUser }: AppointmentFormProps) {
  ...
  useEffect(() => {
    Promise.all([getPatients(), getServices(), getUsers(), getAppointments()]).then(([p, s, u, a]) => {
      ...
      setPerformers(u.filter(x => x.isPerformer && x.active));
      setAppointments(a);
      setLoading(false);
    });
  }, []);
  ...
  return (
    <div dir="rtl">
      <div className="mb-5">
        <h2 className="text-lg font-bold text-slate-800">ثبت نوبت</h2>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <div className="bg-white rounded-2xl border border-slate-100 p-5">
          <h3 className="text-sm font-bold text-slate-700 mb-4">اطلاعات نوبت</h3>
        ...
        {/* Recent appointments */}
        <div>
          <h3 className="text-sm font-bold text-slate-600 mb-3">نوبت‌های اخیر</h3>
          <div className="space-y-2">
            ...
            {appointments.length === 0 && <p className="text-sm text-slate-400 text-center py-8">نوبتی ثبت نشده</p>}
          </div>
        </div>
      </div>
    </div>
